...
export default class HuaTable extends React.Component{
  ...
  render(){
    const {columns, dataSource,bordered, local} = this.props
      ...
      <thead>
          <tr className={styles.table_tr}>
            {columns.map((column, index) => {
              return <th className={ bordered ? styles.table_th_solid :styles.table_th}  key={column.key || index}>{column.title}</th>
            })}
          </tr>
        </thead>
        <tbody>
